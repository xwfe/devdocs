//! Babel 抓取器宏测试
//!
//! 这是一个独立的测试文件，用于测试简化版 Babel 抓取器宏
...
define_scraper!(
    ...
    run_impl: |s: &mut SimplifiedBabelScraper| {
        println!("正在抓取 Babel {} 文档...", s.version);
        println!("基础 URL: {}", s.base_url);
        println!("输出路径: {}", s.output_path);
        
        // 实现抓取逻辑，参照原始 Ruby 项目的实现
        // 1. 下载文档
        let base_url = format!("{}{}/", s.base_url, s.version);
